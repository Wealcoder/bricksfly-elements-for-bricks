...
if (! function_exists('thebrbre_is_license_valid')) {
  ...
  /**
   * Bricksfly has no license/paid tier — every feature is always available.
   * Kept as a function (rather than removing every call site) since it's
   * still used as a general "is Pro installed" signal in a few places.
   *
   * @return bool
   */
...
if (! function_exists('thebrbre_get_license_limitations')) {
  ...
  /**
   * No license tiers exist — every feature key resolves to allowed.
   *
...
if (! function_exists('thebrbre_is_feature_allowed')) {
  ...
  /**
   * Bricksfly has no license/paid tier — every feature is always allowed.
   *
...
if (! function_exists('thebrbre_get_element_bricks_name')) {
   ...
   * that's toggled off never has its real code loaded just to resolve its
   * name for a placeholder.
   *
