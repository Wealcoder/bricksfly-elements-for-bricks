 ...
 * front-end is provided by `public/js/starter-animations.js` and
...
class AAB_Starter_Animations {
	...
	public function enqueue_assets() {
		// Use filemtime() as the cache-buster so browsers pick up edits to
		// the static assets without needing a plugin version bump every time.
		$css_path = AAB_ADDONS_PATH . 'public/css/starter-animations.css';
		$js_path  = AAB_ADDONS_PATH . 'public/js/starter-animations.js';

		$css_ver = file_exists( $css_path ) ? filemtime( $css_path ) : AAB_ADDONS_VERSION;
		$js_ver  = file_exists( $js_path ) ? filemtime( $js_path ) : AAB_ADDONS_VERSION;
		// if (function_exists('bricks_is_builder') && bricks_is_builder()) {
		// 	return;
		// }
		wp_enqueue_style(
			'aab-starter-animations',
			AAB_ADDONS_URL . 'public/build/extensions/starter-animations.css',
			[],
			$css_ver
		);
		...
		wp_enqueue_script(
			'aab-starter-animations',
			AAB_ADDONS_URL . 'public/build/extensions/starter-animations.js',
			[],
			$js_ver,
			true
		);

		// Builder iframe bridge: copies our filter-added `aab-*` classes and
		// `--aab-*` inline styles from Bricks's per-element htmlAttributes map
		// onto the canvas DOM. Text widgets (heading, text, text-basic,
		// text-link) use a Vue contenteditable template that doesn't surface
		// those classes on its own — without this bridge, the public
		// starter-animations.js never finds the element in the builder.
		if ( function_exists( 'bricks_is_builder_iframe' ) && bricks_is_builder_iframe() ) {
			$builder_js_path = AAB_ADDONS_PATH . 'public/js/starter-animations-builder.js';
			$builder_js_ver  = file_exists( $builder_js_path ) ? filemtime( $builder_js_path ) : AAB_ADDONS_VERSION;

			wp_enqueue_script(
				'aab-starter-animations-builder',
