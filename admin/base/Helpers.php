...
class Helpers {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'bricksfly-elements-for-bricks' ),
					'<br>',
					$file_path
				)
			);
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'bricksfly-elements-for-bricks' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'bricksfly-elements-for-bricks' ),
					'<strong>',
					'</strong>',
					sprintf(
						/* translators: 1: opening <strong> tag, 2: closing </strong> tag (around the word "direct"). */
						'<a href="http://gregorcapuder.com/wordpress-how-to-set-direct-filesystem-method/" target="_blank">' . esc_html__( 'How to set %1$sdirect%2$s filesystem method', 'bricksfly-elements-for-bricks' ) . '</a>',
						'<strong>',
						'</strong>'
					)
				)
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'bricksfly-elements-for-bricks' )
			);
		}
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				esc_html__( 'Your user role isn\'t high enough. You don\'t have permission to import demo data.', 'bricksfly-elements-for-bricks' )
			);
		}
	}
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					...
					__( 'Content file was not uploaded. Error: %s', 'bricksfly-elements-for-bricks' ),
					$content_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'bricksfly-elements-for-bricks' )
			);
		}

		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'bricksfly-elements-for-bricks' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'bricksfly-elements-for-bricks' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf(
			...
			__( 'Initial max execution time = %s', 'bricksfly-elements-for-bricks' ),
		...
		sprintf(
			...
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$s', 'bricksfly-elements-for-bricks' ),
			PHP_EOL,
			get_site_url(),
			empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'bricksfly-elements-for-bricks' ) : $selected_import_files['content'],
		);
	}
