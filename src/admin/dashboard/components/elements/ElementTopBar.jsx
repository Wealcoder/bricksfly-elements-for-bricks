...
import { useActiveItem, useElements } from "@/hooks/app.hooks";
...
const ElementTopBar = ({
  ...
  const { updateActiveFullElement } = useActiveItem();

  const setCheck = (data) => {
    updateActiveFullElement(data);
  };
  return (
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center">
            <p className="text-sm text-label ">
              {elementCount?.total} Total Elements
            </p>
            <Dot className="w-4 h-4 text-icon-secondary" strokeWidth={4} />
            <p className="text-sm text-label ">
              {elementCount?.active} Active Elements
            </p>
          </div>
        </div>
      </div>
